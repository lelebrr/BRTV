...
final class TtmlRenderUtil {
    ...
    public static void applyStylesToSpan(Spannable spannable, int i6, int i10, TtmlStyle ttmlStyle, TtmlNode ttmlNode, Map<String, TtmlStyle> map, int i11) {
        TtmlNode ttmlNodeFindRubyTextNode;
        TtmlStyle ttmlStyleResolveStyle;
        int i12;
        if (ttmlStyle.getStyle() != -1) {
            spannable.setSpan(new StyleSpan(ttmlStyle.getStyle()), i6, i10, 33);
        }
        ...
        if (ttmlStyle.hasFontColor()) {
            SpanUtil.addOrReplaceSpan(spannable, new ForegroundColorSpan(ttmlStyle.getFontColor()), i6, i10, 33);
        }
        if (ttmlStyle.hasBackgroundColor()) {
            SpanUtil.addOrReplaceSpan(spannable, new BackgroundColorSpan(ttmlStyle.getBackgroundColor()), i6, i10, 33);
        }
        if (ttmlStyle.getFontFamily() != null) {
            SpanUtil.addOrReplaceSpan(spannable, new TypefaceSpan(ttmlStyle.getFontFamily()), i6, i10, 33);
        }
        if (ttmlStyle.getTextEmphasis() != null) {
            ...
            SpanUtil.addOrReplaceSpan(spannable, new TextEmphasisSpan(i13, i12, i14), i6, i10, 33);
        }
        ...
        if (rubyType == 2) {
            ...
            if (ttmlNodeFindRubyContainerNode != null && (ttmlNodeFindRubyTextNode = findRubyTextNode(ttmlNodeFindRubyContainerNode, map)) != null) {
                if (ttmlNodeFindRubyTextNode.getChildCount() != 1 || ttmlNodeFindRubyTextNode.getChild(0).text == null) {
                    ...
                    spannable.setSpan(new RubySpan(str, rubyPosition), i6, i10, 33);
                }
            }
        } else if (rubyType == 3 || rubyType == 4) {
            spannable.setSpan(new DeleteTextSpan(), i6, i10, 33);
        }
        if (ttmlStyle.getTextCombine()) {
            SpanUtil.addOrReplaceSpan(spannable, new HorizontalTextInVerticalContextSpan(), i6, i10, 33);
        }
        int fontSizeUnit = ttmlStyle.getFontSizeUnit();
        if (fontSizeUnit == 1) {
            SpanUtil.addOrReplaceSpan(spannable, new AbsoluteSizeSpan((int) ttmlStyle.getFontSize(), true), i6, i10, 33);
        } else if (fontSizeUnit == 2) {
            SpanUtil.addOrReplaceSpan(spannable, new RelativeSizeSpan(ttmlStyle.getFontSize()), i6, i10, 33);
        } else {
            if (fontSizeUnit != 3) {
                return;
            }
            SpanUtil.addInheritedRelativeSizeSpan(spannable, ttmlStyle.getFontSize() / 100.0f, i6, i10, 33);
        }
    }
    ...
    public static TtmlStyle resolveStyle(TtmlStyle ttmlStyle, String[] strArr, Map<String, TtmlStyle> map) {
        ...
        if (ttmlStyle == null) {
            ...
            if (strArr.length > 1) {
                ...
                while (i6 < length) {
                    ttmlStyle2.chain(map.get(strArr[i6]));
                    i6++;
                }
                return ttmlStyle2;
            }
            ...
            if (strArr != null && strArr.length > 1) {
                ...
                while (i6 < length2) {
                    ttmlStyle.chain(map.get(strArr[i6]));
                    i6++;
                }
            }
        }
