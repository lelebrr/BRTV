...
public class DefaultTimeBar extends View implements TimeBar {
    ...
    private static int dpToPx(float f, int i6) {
    ...
    private void drawTimeBar(Canvas canvas) {
        ...
        int i6 = iHeight + iCenterY;
        if (this.duration <= 0) {
            Rect rect = this.progressBar;
            canvas.drawRect(rect.left, iCenterY, rect.right, i6, this.unplayedPaint);
            return;
        }
        ...
        if (iMax < i12) {
            canvas.drawRect(iMax, iCenterY, i12, i6, this.unplayedPaint);
        }
        int iMax2 = Math.max(i10, this.scrubberBar.right);
        if (i11 > iMax2) {
            canvas.drawRect(iMax2, iCenterY, i11, i6, this.bufferedPaint);
        }
        if (this.scrubberBar.width() > 0) {
            Rect rect3 = this.scrubberBar;
            canvas.drawRect(rect3.left, iCenterY, rect3.right, i6, this.playedPaint);
        }
        ...
        for (int i14 = 0; i14 < this.adGroupCount; i14++) {
            int iWidth = ((int) ((this.progressBar.width() * Util.constrainValue(jArr[i14], 0L, this.duration)) / this.duration)) - i13;
            Rect rect4 = this.progressBar;
            canvas.drawRect(Math.min(rect4.width() - this.adMarkerWidth, Math.max(0, iWidth)) + rect4.left, iCenterY, r8 + this.adMarkerWidth, i6, zArr[i14] ? this.playedAdMarkerPaint : this.adMarkerPaint);
        }
    }
    ...
    private boolean isInSeekBar(float f, float f3) {
        return this.seekBounds.contains((int) f, (int) f3);
    }
    ...
    private static int pxToDp(float f, int i6) {
        return (int) (i6 / f);
    }
    ...
    private void setSystemGestureExclusionRectsV29(int i6, int i10) {
        Rect rect = this.lastExclusionRectangle;
        if (rect != null && rect.width() == i6 && this.lastExclusionRectangle.height() == i10) {
            return;
        }
    ...
    private void update() {
        ...
        if (this.duration > 0) {
            ...
            int iWidth2 = (int) ((this.progressBar.width() * j10) / this.duration);
            Rect rect3 = this.scrubberBar;
            Rect rect4 = this.progressBar;
            rect3.right = Math.min(rect4.left + iWidth2, rect4.right);
        } else {
            Rect rect5 = this.bufferedBar;
            int i6 = this.progressBar.left;
            rect5.right = i6;
            this.scrubberBar.right = i6;
        }
        invalidate(this.seekBounds);
    }
    ...
    */
    ...
    public void onRtlPropertiesChanged(int i6) {
        Drawable drawable = this.scrubberDrawable;
        if (drawable == null || !setDrawableLayoutDirection(drawable, i6)) {
            return;
        }
        invalidate();
    }
    ...
    */
    ...
    @Override // androidx.media3.ui.TimeBar
    public void setAdGroupTimesMs(long[] jArr, boolean[] zArr, int i6) {
        Assertions.checkArgument(i6 == 0 || !(jArr == null || zArr == null));
        this.adGroupCount = i6;
    ...
    public void setScrubberColor(int i6) {
        this.scrubberPaint.setColor(i6);
        invalidate(this.seekBounds);
    }
    ...
    private static boolean setDrawableLayoutDirection(Drawable drawable, int i6) {
        return Util.SDK_INT >= 23 && drawable.setLayoutDirection(i6);
    }

    public DefaultTimeBar(Context context, AttributeSet attributeSet, int i6) {
        this(context, attributeSet, i6, attributeSet);
    }

    public DefaultTimeBar(Context context, AttributeSet attributeSet, int i6, AttributeSet attributeSet2) {
        this(context, attributeSet, i6, attributeSet2, 0);
    }

    public DefaultTimeBar(Context context, AttributeSet attributeSet, int i6, AttributeSet attributeSet2, int i10) {
        ...
        if (attributeSet2 != null) {
            TypedArray typedArrayObtainStyledAttributes = context.getTheme().obtainStyledAttributes(attributeSet2, R.styleable.DefaultTimeBar, i6, i10);
            try {
