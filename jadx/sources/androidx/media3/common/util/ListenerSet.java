...
public final class ListenerSet<T> {
    ...
    public static final class ListenerHolder<T> {
        ...
        public void invoke(int i6, Event<T> event) {
            if (this.released) {
                return;
            }
            if (i6 != -1) {
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public static /* synthetic */ void lambda$queueEvent$0(CopyOnWriteArraySet copyOnWriteArraySet, int i6, Event event) {
        Iterator it = copyOnWriteArraySet.iterator();
        while (it.hasNext()) {
            ((ListenerHolder) it.next()).invoke(i6, event);
        }
    }
    ...
    public void queueEvent(int i6, Event<T> event) {
        verifyCurrentThread();
        this.queuedEvents.add(new d(new CopyOnWriteArraySet(this.listeners), i6, event, 0));
    }
    ...
    private ListenerSet(CopyOnWriteArraySet<ListenerHolder<T>> copyOnWriteArraySet, Looper looper, Clock clock, IterationFinishedEvent<T> iterationFinishedEvent, boolean z7) {
        ...
        this.handler = clock.createHandler(looper, new Handler.Callback() { // from class: androidx.media3.common.util.c
            @Override // android.os.Handler.Callback
            public final boolean handleMessage(Message message) {
                return this.f2085a.handleMessage(message);
            }
        });
        this.throwsWhenUsingWrongThread = z7;
    }
